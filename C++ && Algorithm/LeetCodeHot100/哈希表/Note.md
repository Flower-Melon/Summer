...
这显然是一个使用哈希表的很好的例子，归根结底，哈希表就是为了快速查找某个值是否在容器中而设计的。

本题只要不断查找当前位置的数据`i + step`值是否存在于容器中，就能找到所有完整的序列

这里注意，可以优化时间复杂度，为了避免重复查找，必须保证查找的`i`是序列的开始元素，也就是保证`i-1`不在容器中即可
